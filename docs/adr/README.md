...
**Decision.** Decimal.js для денежных вычислений, проверенные decimal strings на API/queue границах; явные типы Price, BaseQuantity, QuoteAmount, ContractQuantity и Rate с asset/instrument context. PostgreSQL хранит `numeric(p,s)`, где пределы каждого поля определяются диапазонами инструмента и накоплениями в PHASE 2. Один универсальный scale не предполагается. До записи проверять overflow и scale: не допускать незаметного округления БД. Рабочая precision включает промежуточные произведения/деления; rounding mode и момент quantization документируются по операции. Tick/step применяются как кратность, после quantization повторно проверяются notional и риск.
